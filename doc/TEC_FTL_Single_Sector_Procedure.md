...
Read in the DV xml files for the SPOC TCEs and store the TCE data in a convenient format. Prereq: None. Wait until finished. Output: sector33_20200208_tce.h5 (can check # of TCEs is as expected by checking DAWG ticket)
...
Output the TCE data in a human friendly .txt file. Prereq: Step 1. Wait until finished. Output: sector33_20200208_tce.txt
python dump_tce_info.py
Bin all light curve files from 2min to 10minute. Prereq: None. Continue to next step while this is running. Output: Under the S# directories each light curve will be stored in .h5 format. for TIC 141122198 planet number 2 ~/spocvet/sector33/S33/141122/tess_dvts_0000000141122198_02.h5d
...
Find cadences where many transit ephemerides overlap such that the transits should be deweighted when recalculating significance. Prereq: None. Continue to next step while this is running. Output: Will bring up a figure window. Close figure window. skyline_data_sector33_20200208.txt
python skyline_spoc.py
Match TCE ephemerides to the known planet ephemerides at the exoplanet archive. This routine takes a long time since it needs to query MAST as well. This step can be a bit finicky. Every once in a while the queries to MAST will just hang and/or it will error out. Usually just rerunning the command will result in it working. Prereq: None. Continue to next step while this is running. Output: federate_known_sector33_20200208.txt
...
Match TCE ephemeride list to itself. This finds matches in common indicating systematics. Prereq: None. Continue to next step while this runs. Output: selfMatch_sector33_20200208.txt
python selfMatch_spoc.py
Rip the difference image pages from the DV report. When running this command can sometimes produce ‘GPL Ghostscript…’ warnings and error messages. Those are actually ok. You can run tec_status to see that the DV report pages are actually being generated. Prereq: None. Takes a long time to run so continue to next step while this is running. Output: Difference images in their own pdf for each TCE ~/spocvet/sector33/S33/141122/tess_diffImg_0000000141122198_02_33.pdf
...
Match the TOI ephemerides to the TCEs. Prereq: Wait until Step 5 completes. Output: federate_toiWtce_sector33_20200208.txt
...
Run the triage filter. Prereq: Step 9. Wait until this completes but it is quick. Output: spoc_fluxtriage_sector33_20200208.txt
python flux_triage.py
Perform a trapezoid model fit and run the modshift test for the DV median detrended light curve. Prereq: Step 11. Continue with next step while this runs. Output: Modshift plot outputs for every triage passing TCE and trapezoid fit parameters. ~/spocvet/sector33/S33/140900/tess_0000000140900726_01_med-modshift.pdf & ~/spocvet/sector33/S33/140900/tess_trpzdfit_0000000140900726_01.txt
...
Perform the sweet test. Prereq: Step 11. Continue with next step while this runs. Output: spoc_sweet_sector33_20200208.txt
python sweet_test.py
Gather the flux weighted centroid time series and some PDC statistics about the quality of the light curve. Prereq: Step 11. Continue with next step while this runs. Output: ~/spocvet/sector33/S33/140900/tess_flxwcent_0000000140900726_01.h5d
...
Check whether the TCE events line up with momentum dumps. Prereq: Step 11. Continue with next step while this runs. Output: spoc_modump_sector33_20200208.txt
python modump_check.py
Generate the twexo page that has convenient URL links and GAIA data. Unfortunately this needs to run under a python 3 environment. So, I have a separate python environment installed on PDO which I can help install if you want to run this. Prereq: Step 11. Continue with next step while this runs.
python gen_twexo.py
Run modshift test again, but using the TEC detrended light curve. You may be tempted to start this right after Step 12, but do not do this. There are some temporary files created in modshift that if the two modshift runs work on the same target causes problems. Just run this now after getting the other steps 13-16 started and the Step 12 gets far enough along that this issue won’t happen. Prereq: Step 11. Now we wait for steps 12-17 to finish.
